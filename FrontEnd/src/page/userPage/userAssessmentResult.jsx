...
export default function AssessmentResult() {
  ...
  if (!result) return <p className="text-center p-6 sm:p-8 text-sm sm:text-base">No results found.</p>;
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen">
      <div className="w-full lg:w-1/2 bg-[#9BB168] flex flex-col items-center justify-center text-white text-center px-4 sm:px-6 py-8 lg:py-0">
        <img src={Bot} alt="Bot" className="w-40 h-40 sm:w-48 sm:h-48 lg:w-56 lg:h-56 mb-4 sm:mb-6" />        <h2 className="text-2xl sm:text-3xl font-bold mb-2">Assessment Results</h2>
        <p className="text-base sm:text-lg mb-4">Your overall status today is:</p>
        <p className="text-xl sm:text-2xl font-bold mb-2">{result.overallStressLevel}</p>
        <p className="text-base sm:text-lg mb-6">
          Average score: {Math.round(result.averageStressScore)}
        </p>
      ...
      <div className="w-full lg:w-1/2 bg-yellow-100 p-4 sm:p-6 lg:p-10">
        <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-brown-700">Answer Details</h3>
        <ul className="space-y-4 sm:space-y-6">
          {result.answers.map((ans, idx) => (
            ...
            >
              ...
              </p>              <p className="text-xs sm:text-sm text-green-700">
                Answer: <span className="font-medium">{ans.selectedOptionText}</span>
              </p>
              <p className="text-xs sm:text-sm text-gray-600">
                Level: <span className="font-medium">{ans.stressLevel}</span> - Score: {Math.round(ans.stressScore)}
              </p>
            </li>
          ))}
        </ul>
