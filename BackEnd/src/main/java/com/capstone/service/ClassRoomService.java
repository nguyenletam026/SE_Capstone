...
public class ClassRoomService {
    ...
    public List<StudentStressDto> getStudentsStressData(String classId) {
        ...
        for (User student : classRoom.getStudents()) {
            // Get the latest stress analysis for each student
            List<StressAnalysis> stressAnalyses = stressAnalysisRepository.findByUserOrderByCreatedAtDesc(student);
            StressAnalysis latestAnalysis = stressAnalyses.isEmpty() ? null : stressAnalyses.get(0);
            
            StudentStressDto dto = StudentStressDto.builder()
                    .studentId(student.getId())
                    .studentName(student.getFirstName() + " " + student.getLastName())
                    .username(student.getUsername())
                    .stressLevel(latestAnalysis != null ? latestAnalysis.getStressLevel() : "NO_DATA")
                    .lastUpdated(latestAnalysis != null ? latestAnalysis.getCreatedAt() : null)
                    .build();
